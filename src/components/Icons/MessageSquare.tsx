export function MessageSquare(){
    return(
        <svg width="15" height="16" viewBox="0 0 15 16" fill="none" xmlns="http://www.w3.org/2000/svg">
        <path d="M13.125 9.875C13.125 10.2065 12.9933 10.5245 12.7589 10.7589C12.5245 10.9933 12.2065 11.125 11.875 11.125H4.375L1.875 13.625V3.625C1.875 3.29348 2.0067 2.97554 2.24112 2.74112C2.47554 2.5067 2.79348 2.375 3.125 2.375H11.875C12.2065 2.375 12.5245 2.5067 12.7589 2.74112C12.9933 2.97554 13.125 3.29348 13.125 3.625V9.875Z" stroke="#808080" strokeLinecap="round" strokeLinejoin="round"/>
        </svg>
    )
}